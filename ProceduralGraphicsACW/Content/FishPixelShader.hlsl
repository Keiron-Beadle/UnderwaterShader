...
{
	float3 rgb = normalize(input.texcoord.grr + (input.texcoord.grg * 0.35) + input.position.xyz * 0.001);
	
	float2 centre = float2(0.5f, 0.5f);
	float dist = length(input.texcoord - centre);

	float alpha = smoothstep(0.1f, 0.5f, dist);
	if (1 - alpha < 0.01) { discard; }
	...
	diffuse *= caustic(float2(lerp(input.position.x, input.position.y, 0.2), lerp(input.position.z, input.position.y, 0.2)) * 1.1);
	float3 lightCol = 1.0 * sunLightColour;
	lightCol += 0.7 * sky * skyLightColour;
	colour *= 2.12 * diffuse * lightCol;
	colour = lerp(colour, horizonColour, 1.0 - exp(-0.3 * pow(0.6, 1.0)));
	float3 gamma = (float3).53;

	return float4(pow(diffuse, gamma), 1-alpha);
}